...
This development plan outlines the implementation strategy for building the Cost of Travel Index, a county-level economic analysis tool that calculates the cost of a hypothetical three-day weekend across U.S. counties using Zartico's BigQuery datasets.
...
- **Data Sources**: BigQuery tables (transaction_tourism, merchant_tourism, admin_geo_reference, plus lodging data TBD)
...
- **DC FIPS Issue**: Manual handling required to join spend and lodging data for Washington DC
...
- **Basket Quantities**:
  - `lodging_nights` (int): Default 2 (Friday & Saturday)
...
- **Quality Thresholds** (initial estimates, to be refined in Phase 4):
  - `min_hotel_properties` (int): Default 3
