-- ============================================================================
-- RETAIL: County-Level Cost Calculation WITH DAILY TRANSACTION CAP
-- ============================================================================
-- Purpose: Calculate median per-transaction retail spending at the county level
...
-- Outlier Removal: P5/P98 per county (applied AFTER capping)
...
cardholder_shopping_patterns AS (
  ...
  FROM (
    ...
    FROM retail_transactions_raw
    GROUP BY merch_county, merch_state, month_date, membccid, trans_date
  )
  GROUP BY merch_county, merch_state, month_date, membccid
),

-- Track capping impact per county with better shopping behavior metrics
capping_impact AS (
  SELECT
    r.merch_county,
    r.merch_state,
  ...
  JOIN cardholder_shopping_patterns p
    ON r.merch_county = p.merch_county
    AND r.merch_state = p.merch_state
    AND r.month_date = p.month_date
    AND r.membccid = p.membccid
  GROUP BY r.merch_county, r.merch_state, r.month_date
),

-- Calculate P5 and P98 thresholds per county per month for outlier removal (AFTER capping)
county_thresholds AS (
  SELECT
    merch_county,
    merch_state,
    month_date,
  ...
  GROUP BY merch_county, merch_state, month_date
),
...
retail_no_outliers AS (
  ...
  JOIN county_thresholds t
    ON r.merch_county = t.merch_county
    AND r.merch_state = t.merch_state
...
LEFT JOIN capping_impact i
  ON r.merch_county = i.merch_county
  AND r.merch_state = i.merch_state
  AND r.month_date = i.month_date
GROUP BY r.merch_county, r.merch_state, r.month_date, i.txn_before_cap, i.txn_after_cap,
