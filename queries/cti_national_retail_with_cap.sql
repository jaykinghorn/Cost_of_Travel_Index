-- ============================================================================
-- CTI NATIONAL RETAIL: National-Level Cost Calculation WITH DAILY CAP
-- ============================================================================
...
-- Outlier Removal: P5/P98 nationally per month (applied AFTER capping)
-- Date Range: Configurable
--
-- Rationale: National aggregation provides a single U.S. baseline for retail
--            costs. Using individual transactions with daily cap provides a
--            representative sample while preventing high-frequency shoppers
--            from inflating the median.
--
-- MCC Codes: 38 retail codes covering art supplies, cosmetics, books, sporting goods,
--            apparel, jewelry, gift shops, department stores, and specialty retail
-- ============================================================================
...
WITH retail_transactions_raw AS (
  -- Extract all retail transactions with daily ranking (no state grouping)
  SELECT
...
cardholder_shopping_patterns AS (
  ...
  FROM (
    ...
    FROM retail_transactions_raw
    GROUP BY month_date, membccid, trans_date
  )
  GROUP BY month_date, membccid
),

-- Track capping impact nationally with shopping behavior metrics
capping_impact AS (
  SELECT
    r.month_date,
  ...
  JOIN cardholder_shopping_patterns p
    ON r.month_date = p.month_date
    AND r.membccid = p.membccid
  GROUP BY r.month_date
),

-- Calculate P5 and P98 thresholds nationally per month for outlier removal (AFTER capping)
...
LEFT JOIN capping_impact i
  ON r.month_date = i.month_date
GROUP BY r.month_date, i.txn_before_cap, i.txn_after_cap,
